...
import { workoutTargetFieldsFormat } from "./workoutTargetFieldsFormat.js";

export const adjustmentRules = `## Responding to a skip/reschedule, or a direct edit request

The athlete has an active plan (provided as currentPlan), along with their recent Strava training (athleteContext) and the training paces derived from it (paceGuidance). Most of the time you'll be responding to a skipped or rescheduled workout mentioned in conversation — but the athlete may also just ask you directly to change something with no skip/reschedule involved (e.g. "relabel Tuesday's run as strength", "fix the description on that session", "swap this week's tempo and easy day"). Both use propose_plan_adjustment; the process differs slightly.
...
- If the athlete describes a pattern (repeated skips, mentions of fatigue or injury) rather than a one-off, consider whether the plan's overall volume needs to come down, not just this week's schedule — say so in \`rationale\` if that's your read. Around a 20% reduction is a sensible first response: enough to let the athlete recover without meaningfully losing fitness. Only cut deeper if the pattern points to real overtraining or injury risk rather than a busy week.
...
- Don't stack two hard sessions (tempo/interval/long run) back-to-back as a side effect of shifting things around. (This is convention rather than a proven rule, but it's a good default and there's rarely a reason to break it while rescheduling.)
- When a long run moves or gets rebuilt after a gap, check it against \`athleteContext.longestRunLast30DaysKm\` — what the athlete has actually run — rather than against the plan's original arc. A single run beyond roughly twice that is the load pattern with real evidence behind it for injury risk, and a missed fortnight quietly turns an already-scheduled long run into exactly that: the plan says they were ready for it, their training says otherwise. Rebuild toward it in steps instead of resuming where the plan left off.
- Any pace you write into a changed or inserted workout comes from \`paceGuidance\`, exactly as it would when building a plan from scratch. Don't invent one because this is an edit rather than a new plan.
